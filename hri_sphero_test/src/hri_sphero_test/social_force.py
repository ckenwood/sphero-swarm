...
class Sphero(object):
	...
	def agent_repulse(self, other):
		b = math.sqrt((self.xpos - other.xpos)**2 + (self.ypos - other.ypos)**2)
	...
	def set_pose(self, msg):
        ...
        self.xpos = msg.pose.point.y
        self.vx_imu = msg.twist.linear.x
        self.vy_imu = msg.twist.linear.y

	def _init_pubsub(self):
        self.cmd_vel_pub = rospy.Publisher(self.name + 'cmd_vel', Twist, self.cmd_vel, queue_size = 1)
        self.odom_sub = rospy.Sublisher(self.name + 'odom', Odometry, self.set_pose)

    def pub_cmd(self):
    	# To run publishers subscribers for sending velocity command to a sphero
...
if __name__ == '__main__':
    ...
    # Need to write the code to publish the velocities 
    for obj in spheros:
    	obj.pub_cmd()
